...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        ...
        int colNameIndex = mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_GUEST_NAME);
        String guestName = mCursor.getString(colNameIndex);

        // COMPLETED (7) Call getInt on the cursor to get the party size
        int colPartyIndex = mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_PARTY_SIZE);
        String partySize = mCursor.getString(colPartyIndex);
