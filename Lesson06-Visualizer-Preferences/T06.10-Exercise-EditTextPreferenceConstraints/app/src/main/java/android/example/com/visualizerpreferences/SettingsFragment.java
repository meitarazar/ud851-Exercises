...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (key.equals(getString(R.string.pref_size_key))) {
            ...
            try {
                scale = Float.valueOf(newValue.toString().trim());
            } catch (Exception e) {
                Toast.makeText(getContext(), "That is not a valid float!", Toast.LENGTH_LONG).show();
                return false;
            }

            if (scale <= 0 || scale > 3) {
                Toast.makeText(getContext(), "Please enter a float between 0 (exclusive) and 3 (inclusive)", Toast.LENGTH_LONG).show();
                return false;
            }
        }
